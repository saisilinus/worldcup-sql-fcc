...
echo "$($PSQL "SELECT name FROM games AS g LEFT JOIN teams as t ON g.winner_id = t.team_id WHERE year = 2014 AND round = 'Eighth-Final' UNION SELECT name FROM games AS g LEFT JOIN teams as t ON g.opponent_id = t.team_id WHERE year = 2014 AND round = 'Eighth-Final'")"
